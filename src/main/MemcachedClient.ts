...
export class MemcachedClient extends EventEmitter {
    ...
    constructor(servers: Servers, options: Partial<IMemcachedConfig> = {}) {
        super()
        this._config = Utils.merge(MemcachedClient.config, options)
    ...
    public touch(
        key: string,
        ttl: number = this._config.defaultTTL,
    ): Promise<boolean> {
    ...
    public set(
        ...
        return this._setters('set', key, value, ttl)
    }
    ...
    public add(
        ...
        return this._setters('add', key, value, ttl)
    }
    ...
    public cas(
        ...
        return this._setters('cas', key, value, ttl, cas)
    }
    ...
    public get<T = any>(key: string | Array<string>): Promise<T> {
        if (Array.isArray(key)) {
            ...
            const fullkey = `${this._config.namespace}${key}`
            return this._executeCommand(
                (noreply: boolean | undefined): CommandOptions => ({
                    type: 'get',
                    key: fullkey,
                    validate: [['key', String]],
                    command: `get ${fullkey}`,
    ...
    public getOrElse<T = any>(key: string, fn: () => Promise<T>): Promise<T> {
        return this.get<T>(key).catch((err: any) => {
            console.log('err: ', err)
            return fn().then((val: T) => {
                return this.set(key, val).then(() => {
                    return val
                })
            })
        })
    ...
    public cachedump(
        ...
        return this._executeCommand(
            () => ({
                ...
                number: num,
                slabid,
                validate: [['number', Number], ['slabid', Number]],
            ...
            results.forEach((next: any) => {
                ...
                ;(Array.isArray(next) ? next : [next]).forEach((value: any) => {
                    if (value && this._config.namespace.length) {
                        ...
                        newvalue[nsKey.replace(this._config.namespace, '')] =
                            value[nsKey]
    ...
    private _multi(
        ...
        while (i--) {
            promises.push(
                op(servers[i], serverMap[servers[i]], i, servers.length),
            )
        }
    ...
    private _incrdecr(
        ...
        const fullkey = `${this._config.namespace}${key}`
        return this._executeCommand((noreply?: boolean) => ({
            type,
            key: fullkey,
            value,
            ...
            redundancyEnabled: true,
            command:
                [type, fullkey, value].join(' ') + (noreply ? NOREPLY : ''),
        }))
    }
        ...
        if (length > this._config.maxValue) {
            ...
            return this._executeCommand(
                (noreply?: boolean): CommandOptions => ({
                    ...
                    command:
                        `${type} ${fullKey} ${flag} ${lifetime} ${length}` +
                        (cas ? ` ${cas}` : '') +
                        (noreply ? NOREPLY : '') +
                        LINEBREAK +
                        value,
    ...
    private async _executeCommand(
        ...
        if (
            ...
            if (argsError !== undefined) {
                ...
                const redundancy =
                    this._config.redundancy < this._servers.length
                ...
                if (
                    ...
                    return this._connect(server)
                        .then((socket: MemcachedSocket | undefined) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (server in this._issues && this._issues[server].failed) {
                ...
                if (this._connections[server] === undefined) {
                    ...
                    const serverHosts: Array<string> =
                     ...
                     */
                    const manager = new Jackpot<MemcachedSocket>(
                        this._config.poolSize,
                    )
                ...
                // connection
                memcached._connections[server].pull(
                    (err?: Error, socket?: MemcachedSocket) => {
                        if (err) {
                            reject(err)
                        } else {
                            resolve(socket)
    ...
    private _rawDataReceived(
        ...
        while (
            ...
            if (
                tokenType === 'VALUE' &&
                socket.bufferArray.indexOf('END') === -1
            ) {
                ...
                if (TOKEN_TYPES.indexOf(tokenType) > -1) {
                    ...
                    if (tokenType === 'VALUE') {
                        dataSet = Utils.unescapeValue(
                            socket.bufferArray.shift() || '',
                        )
                    }

                    const result: IParseResult = Parser.parse(
                        tokenType,
                        socket,
                        tokenSet,
                        dataSet,
                    ...
                    switch (result.type) {
                        ...
                        default: {
                            ...
                            throw new Error(
                                `Unknown command returned from parse ${_exhaustiveCheck}`,
                            )
                        }
                    }
                } else {
