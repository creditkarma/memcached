...
describe('Memcached connections', () => {
            ...
            (err1: Error) => {
                return memcached.get('idontcare').then(
                    ...
                    (err: Error) => {
                        assert.equal(
                            err.message,
                            'Server at 127.0.1:1234 not available',
                        )
                        memcached.end()
                    },
                )
            ...
            (err1: Error) => {
                ...
                return memcached.get('idontcare').then(
                    ...
                    (err2: Error) => {
                        assert.equal(
                            err2.message,
                            'Server at 127.0.0.1:1234 not available',
                        )
                        assert.equal(
                            (memcached as any)._issues[server].config.failures,
                            5,
                        ...
                        assert.equal(
                            (memcached as any)._issues[server].locked,
                            true,
                        )
                        assert.equal(
                            (memcached as any)._issues[server].failed,
                            true,
                        )
                        ...
                        common.wait(500).then(() => {
                            ...
                            return memcached.get('idontcare').then(
                                ...
                                (err3: Error) => {
                                    ...
                                    assert.equal(
                                        (memcached as any)._issues[server]
                                            .config.failures,
                                        4,
                                    )
                                    memcached.end()
    ...
    it('should default to port 11211', async () => {
        ...
        return memcached.get('idontcare').then((val: any) => {
            memcached.end()
            assert.equal(
                Object.keys((memcached as any)._connections)[0],
                '127.0.0.1:11211',
            )
        })
    })
    ...
    it('should not create multiple connections with no port', async () => {
        ...
        return memcached.get('idontcare').then(() => {
            ...
            return memcached.get('idontcare').then(() => {
                memcached.end()
                assert.equal(
                    (memcached as any)._connections['127.0.0.1:11211'],
                    conn,
                )
            })
        })
    ...
    it('should remove connection when idle', async () => {
        ...
        return memcached.get('idontcare').then(() => {
            assert.equal(
                (memcached as any)._connections[common.servers.single].pool
                    .length,
                1,
            )
    ...
    it('should remove server if error occurs after connection established', async () => {
        ...
        return memcached.get('idontcare').then(() => {
            ...
            const socket = (memcached as any)._connections[
                common.servers.single
            ].pool.pop()
            ...
            return memcached.get('idontcare').then(
                ...
                (err2: Error) => {
                    assert.equal(
                        err2.message,
                        'Server at localhost:11211 not available',
                    )
                },
            )
        })
            ...
            (err: Error) => {
                ...
                return common.wait(15).then(() => {
                    ...
                    return common.wait(300).then(() => {
                        memcached.end()
                        assert.deepEqual(
                            JSON.parse((memcached as any)._issues[server].args)
                                .failures,
                            (memcached as any)._issues[server].config.failures,
                        )
                    })
